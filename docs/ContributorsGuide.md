...
**Note**: While the formatting check is advisory and won't fail your PR, please follow the formatting standards to maintain code consistency by running the local clang-format commands shown above over `include/`, `examples/`, and `tests/`.
...
**Prevention**: Install the provided git hooks (see the install script in `./checklists/`) so the pre-commit hook automatically regenerates docs, or add this step to your commit checklist
...
**Reference**: See [ProgrammersGuide.md](ProgrammersGuide.md) Proxy section
